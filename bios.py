...
def launch_bios(computer_name="WINLAND-PC"):
    ...
    # ===== POST Screen =====
    post = tk.Label(root, text="Press DEL to enter setup...", fg="white", bg="#000080", font=("Courier", 24))
    post.pack(expand=True)
    root.update()
    time.sleep(2)
    post.config(text="Loading Winland BIOS...")
    root.update()
    time.sleep(1)
    post.pack_forget()
    ...
    # ===== BIOS Menu =====
    menus = {
        "Main": ["System Time", "System Date", "System Info", "Advanced", "Boot", "BIOS Update", "Reset BIOS", "Exit"],
    ...
    top_label = tk.Label(root, text="   Main    Advanced    Boot    Exit   ", fg="white", bg="#4B0082", font=("Courier", 18, "bold"))
    top_label.pack(fill="x")

    label = tk.Label(root, text="", fg="white", bg="#000080", font=("Courier", 20), justify="left", anchor="nw")
    label.pack(padx=50, pady=50, anchor="nw")

    # نسخه پایین صفحه
    version_label = tk.Label(root, text=f"BIOS Version: {get_current_version()}", fg="white", bg="#000080", font=("Courier", 16))
    version_label.pack(side="bottom", pady=5)

    # ساعت پایین صفحه
    clock_label = tk.Label(root, fg="white", bg="#000080", font=("Courier", 16))
    ...
    def boot_priority_prompt(device):
        ...
        def yes():
            print(f"{device} set as first boot device (fake)")
            choice.destroy()
        def no():
            choice.destroy()
    ...
    def reset_bios():
        ...
        error.configure(bg="black")
        error.bind("<Escape>", lambda e: error.destroy())
        tk.Label(error, text="BIOS RESET IN PROGRESS...\nCODE: 1010101010", fg="red", bg="black", font=("Courier", 28)).pack(expand=True)
        error.update()
        time.sleep(3)
        error.destroy()
    ...
    def update_bios():
        ...
        tk.Label(loader, text=f"Installing BIOS Update {new_code}...", fg="white", bg="black", font=("Courier", 28)).pack(pady=50)
        ...
        try:
            exec(new_code)  # 🛠 اجرای کد تغییرات ابدیت
    ...
    def select_option(event):
        ...
        if current_menu in ["System Info", "BIOS Update", "Reset BIOS"]:
